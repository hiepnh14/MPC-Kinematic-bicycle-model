...
def template_model(symvar_type='SX'):
    ...
    # x_next[0, 0] = _x[0, 0] + _x[3, 0] * cos(_x[2, 0]) * dt
    # x_next[1, 0] = _x[1, 0] + _x[3, 0] * sin(_x[2, 0]) * dt
    # x_next[2, 0] = _x[2, 0] + _x[3, 0] * tan(_u[1, 0])/3.0 * dt
    # x_next[3, 0] = _x[3, 0] + _u[0, 0] * dt
    model.set_rhs('x', x_next)
    model.setup()

    # x_next = A @ _x + dt * np.array([_u[0, 0] * cos(_x[2, 0])], [_u[0, 0] * sin(_x[2, 0])], [_u[0, 0] * tan(_u[1, 0]) / 1])
    # model.set_rhs('x', x_next)
    # model.set_alg('x_next', x_next - A @ _x - B @ _u)
    # model.set_alg('x_next', x_next-A@_x-np.array([_u[0,0]*cos(_x[2,0])], [_u[0,0]*sin(_x[2,0])], [_u[0,0]*tan(_u[1,0])/1]))
    # x_next = A @ _x + B @ _u
