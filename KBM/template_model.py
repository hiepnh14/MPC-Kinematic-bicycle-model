...
def template_model(symvar_type='SX'):
    ...
    # [a, delta]
    #Set expression
    # the vehicle runs along x-axis y =2 at constant speed
    model.set_expression(expr_name='cost', expr=sum1(((_x[1] - 2) ** 2 + (_x[3] - 0.1) ** 2  )))

    # the vehicle runs along a circle (2, 2) radius 2
    # model.set_expression(expr_name='cost', expr=sum1((((_x[1] - 2) ** 2 + (_x[0] - 2) ** 2 - 9) ** 2 + (_x[3] - 0.2) ** 2)))
